...
Die Aufgabe von Papyrologen ist es, diese Fragmente zusammenzusetzen und zu entziffern. Zusammen mit Historikern können dadurch wichtige Ergebnisse über das Altertum gewonnen werden. Allerdings ist der Prozess des händischen Zusammensetzens sehr zeitaufwendig und schwierig. %TODO Du könntest noch schreiben warum es schwierig ist
\\\\
In der folgenden Arbeit geht es darum, Papyrologen die Arbeit zu erleichtern, indem man den Prozess des Zusammensetzens teilautomatisiert. %TODO "man" streichen bzw umschreiben
Hierfür wird ein Algorithmus entworfen, der zu einem Papyrus-Fragment eine kleinere Auswahl an möglicherweise passenden Fragmenten vorschlägt. %TODO vllt nicht möglicherweise passend sondern mit hoher Wahrscheinlichkeit passend
Zusammengefasst wird dieser Algorithmus im folgenden %TODO im Folgendem großschreiben
eine Puzzlehilfe Funktion genannt. %TODO ich glaube Puzzlehilfe-Funktion
Mit solch einer Puzzlehilfe Funktion kann die Effektivität und Effizienz beim Zusammensetzten von Fragmenten durch Papyrologen erhöht werden. %TODO nicht durch sondern für oder?
Allerdings muss eine Puzzlehilfe Funktion dabei mehre Bedingungen erfüllen.
\\\\
Zum einen darf das Erstellen einer solchen Funktion keine großen Datenmengen an zusammengesetzten Payri Fragmenten voraussetzen, denn es existieren weltweit nur wenige solcher Daten. Deep Metric Lerarning (DML) wurde bisher verwendet, um eine Puzzlehilfe Funktion zu erstellen und erzieht %TODO erziehlt
dabei eine gute Genauigkeit. %TODO was ist eine gute Genauigkeit
Wenn man das DML weiter verfeinern möchte, um eine sehr gute Genauigkeit zu erreichen, werden immens viele Daten benötigt. Andernfalls würde das Model sehr wahrscheinlich nicht konvergieren. Wenn man die Fragmente händisch zusammensetzen würde, um somit einen großen Datensatz zu erhalten, wäre der Zweck der Puzzlehilfe Funktion obsolet. %TODO man streichen
Der Zeitaufwand für den Papyrologen bleibt im besten Fall derselbe oder wird im schlechtesten Fall erhöht. Generative-Advesarial-Networks (GANs) sind eine gut erforschte Möglichkeit, domänenübergreifend mehr künstliche Daten zu erzeugen. Es wurde gezeigt, dass die Genauigkeit über verschiedene Anwendungsbereiche hinweg dadurch signifikant erhöht werden kann. Unser Ziel ist es diese Methode zu verwenden, um die Hürde von zu wenig Daten zu überwinden. %TODO Schreibt man unser? wenn dann dein Ziel

\\\\
Eine andere Bedingung ist, dass die falsch-negativen Treffer möglichst gering sind. In der Vorauswahl der Puzzlehilfe Funktion sollen also möglichst alle potenziell passenden Fragmente enthalten sein. Ist das passende Fragment nicht in der Teilmenge enthalten, hat der Papyrologe keine Chance, das richtige Fragment zu identifizieren. Es sei denn er weitet seine Suche wieder auf den gesamten Datensatz aus, was die Puzzlehilfe Funktion wieder obsolet werden lässt. %TODO: gendern! er/sie
Eine weiter %TODO weiterer
Nachteil ist, dass er möglicherweise extrem lange nach dem passenden Fragment sucht, welches aber nicht zu finden ist. 
Zielführende Metriken, um diese Eigenschaft zu Messen und ein Gesamtbild über das Model zu gewinnen, sind die mAP, top-1, pr@10, pr@100. Im Folgenden sprechen wir %TODO sprechen wir
...
Deshalb soll unser %TODO unser
Ansatz statistisch untersuchen, wie bestimmte Papyruscharakteristiken die Genauigkeit eines solchen Models beeinflussen. Hierfür vergleichen wir %TODO wir
die Genauigkeit verschiedner %TODO verschiedener
Modelle, indem nur der Vordergrund eines Papyrus (Text) oder nur %TODO ein nur ist genug
der Hintergrund (Fasern) als Eingabe zum trainieren verwendet wird. Das separieren von Text und Phasern ist eine ganz %TODO ganz streichen 
eigene Herausforderung mit vielen Hindernissen. %TODO mit vielen Hindernissen umgangsspr.
